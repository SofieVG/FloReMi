...
# Step 4b - Random Survival Forest regression  #
################################################
...
fitGLMNETModel <- function(survivalTime,status,features){
  fit <- glmnet(as.matrix(features),Surv(survivalTime,status),family="cox")
  return(fit)
}

getLambda <- function(survivalTime,status,features){
  cv.fit <- cv.glmnet(as.matrix(features),Surv(survivalTime,status),family="cox")
...
for (i in train_ids) {
  ...
  fit <- fitGLMNETModel(train_survivalTime[fold],train_status[fold],features[fold,selected_features])
  lambda <- getLambda(train_survivalTime[fold],train_status[fold],features[fold,selected_features])
  
  toPredict <- list(time=train_survivalTime[i],status=train_status[i],x=features[i,selected_features,drop=F])
  predictedvalue <- predict(fit,features[i,selected_features,drop=F],s=lambda,type="response", na.action="na.impute")
  prediction <- c(prediction,predictedvalue)
...
fit <- fitGLMNETModel(train_survivalTime,train_status,features[train_ids,selected_features])
lambda <- getLambda(train_survivalTime,train_status,features[train_ids,selected_features])

prediction <- predict(fit,features[,selected_features],s=lambda,type="response", na.action="na.impute")
meta_patients[,"Prediction"] <- prediction
write.csv(meta_patients,file=paste(FCSloc,"dambi","predictionGlmnet.csv",sep="/"))
